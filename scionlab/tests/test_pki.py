...
class GenerateTRCTests(TestCase):
    ...
    def test_update_offline(self):
        ...
        # and all updated keys (proof of posession).
        signing_keys_v2 = [
            (as_id, 'voting_offline', k.voting_offline) for as_id, k in primary_ases_v1.items()
        ] + [
            (as_id, 'voting_offline', k.voting_offline) for as_id, k in primary_ases_v2.items()
        ]
        self.assertTrue(trcs.verify(trc_v2, signing_keys_v2))
    ...
    def test_update_online(self):
        ...
        primary_ases_v2 = copy.deepcopy(primary_ases_v1)
        primary_ases_v2[_ASID_1].voting_online = _gen_key(2)

        trc_v2 = self.gen_trc_update(primary_ases_v2, trc_v1, voting_offline_v1)

        # Updating online keys is a regular update and is signed with online keys
        voting_online = {as_id: {'voting_online': keys.voting_online}
                         for as_id, keys in primary_ases_v2.items()}

        self.assertTrue(trcs.verify(trc_v2, voting_online))
    ...
    def test_update_issuing(self):
        ...
        primary_ases_v2 = copy.deepcopy(primary_ases_v1)
        primary_ases_v2[_ASID_1].issuing_grant = _gen_key(2)

        trc_v2 = self.gen_trc_update(primary_ases_v2, trc_v1, voting_offline_v1)

        # Updating issuing grant keys is a regular update and is signed with online keys and all
        # updated keys (proof of posession).
        voting_online = {as_id: {'voting_online': keys.voting_online}
                         for as_id, keys in primary_ases_v2.items()}
        signing_keys = voting_online
        signing_keys[_ASID_1]['issuing_grant'] = primary_ases_v2[_ASID_1].issuing_grant

        self.assertTrue(trcs.verify(trc_v2, signing_keys))
