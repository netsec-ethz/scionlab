...
def bump_version(apps, schema_editor):
    ...
    """
    css = Service.objects.filter(type=Service.CS, AS__is_core=True)
    hosts = Host.objects.filter(services__type=Service.CS, AS__is_core=True)
    if hosts.count() != css.count():
        raise RuntimeError(f'expected {css.count()} hosts, but got {hosts.count()}')
